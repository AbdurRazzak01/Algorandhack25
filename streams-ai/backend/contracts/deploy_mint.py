...
def compile_program(client, source_path):
    ...
    print("Compiler Response:", response)  # helpful debug
    return base64.b64decode(response["result"])

# 🚀 Deploy the app
def deploy_app():
    approval = compile_program(client, "contracts/mint_approval.teal")
    clear = compile_program(client, "contracts/mint_clear.teal")

    # ✅ FIXED: allow for multiple global ints / bytes
    ...
    print(f"⏳ Deploying MintRedeem App... TXID: {txid}")
    result = wait_for_confirmation(client, txid, 4)
    app_id = result["application-index"]

    print(f"✅ MintRedeem App deployed successfully! App ID: {app_id}")

    # Save for frontend
    with open("contracts/mint_app_id.txt", "w") as f:
        f.write(str(app_id))
